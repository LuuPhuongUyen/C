...
FLIGHT LoadFlight(string fn)
{
    ...
    {
        //     fscanf(fn, "%[^ ]", F.DB[f].from);
        //     fscanf(fn, "%[^ ]", F.DB[f].to);
        //     fscanf(fn, "%d[^]", F.DB[f].noseat);
        //     fscanf(fn, "\n");
        flight flg;
...
void DisplaySeat(FLIGHT F)
{
    for (int d = 0; d < MDAY; d++)
    {
        printf("\n\nDay %d: ", d);
        for (int f = 0; f < F.noflight; f++)
            printf("\n\nFrom %s to %s with %d seats", F.DB[f].from, F.DB[f].to, F.seat[d][f]);
    }
}
int main()
{
    FLIGHT F = LoadFlight("D:\\C\\LAB\\flight.txt");
    Display(F);
    InitSeat(&F);
    DisplaySeat(F);
    int choice;
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        for (f_back = 0; f_back < F.noflight; f_back++)
                            if (!strcmp(F.DB[f_back].from, g.from) && !strcmp(F.DB[f_back].to, g.to))
                                break;
