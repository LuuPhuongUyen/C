...
typedef struct QuesType
{
    ...
    int point;
    string ques;
    int numChoices; //MC Multi Choice
    string *choiceList;
    string ans;
} QuesType;
QuesType *ReadFile(string fn, int *n);
void display(QuesType *QB, int n);
void displayQuestion(QuesType question);
int checkAnswer(QuesType question, string answer);
QuesType *ReadFile(string fn, int *n)
{
    FILE *f = fopen(fn, "r");
    fscanf(f, "%d\n", n);
...
{
    int point = 0;
    if (strcmp(answer, question.ans) == 0)
    {
...
int main()
{
    QuesType *QuesBank;
    int numQues;
    QuesBank = ReadFile("D:\\C\\LAB\\cl4.txt", &numQues);
    //display(QuesBank, numQues);
